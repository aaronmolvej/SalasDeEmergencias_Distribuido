...
# OJO: Asegúrate que el nombre del archivo coincida (detector_fallas vs detector_failure)
...
def on_me_convierto_en_maestro():
    ...
    # 1. Iniciar servicio Maestro (para recibir clientes)
    t_maestro = threading.Thread(
        target=start_master_listener, 
        args=(my_node_config['port_manager'], my_node_id), # <--- Pasamos my_node_id
        daemon=True
    )
    t_maestro.start()
    
    # 2. Actualizar detector: Ahora yo vigilo a los esclavos
    if detector:
...
def main(node_id):
    ...
    # Si no existe, la creamos (útil para nodos nuevos)
    if not os.path.exists(ruta_db): 
        DatabaseManager(ruta_db, SCHEMA_PATH)

    # 3. SERVICIO DE ALMACENAMIENTO (Siempre activo, puerto 900X)
